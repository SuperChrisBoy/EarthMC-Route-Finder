...
public final class AccessibilitySavesScreen extends Screen {
    ...
    @Override public void render(GuiGraphics g,int mx,int my,float delta){
        ...
        g.drawString(font,font.plainSubstrByWidth("Current: "+(active.isBlank()?"Unsaved reports":active),span),left,29,0xFF8BE5BB,false);
        g.drawString(font,font.plainSubstrByWidth("Load replaces town/nation spawn reports only. Save current first to keep them.",span),left,96,0xFFB9C5CB,false);
        if(entries.isEmpty())g.drawString(font,"No saves yet. Name and save your current reports above.",left,118,0xFFB9C5CB,false);
        g.drawString(font,font.plainSubstrByWidth(status,span),left,height-64,0xFFFFCC66,false);
        g.drawCenteredString(font,(page+1)+" / "+Math.max(1,(entries.size()+rows-1)/rows),width/2,height-42,0xFFB9C5CB);
    }
