...
public final class TeleportAccessService {
    ...
    public synchronized void setSpawnReport(TeleportDestination d,TeleportDestination.PhysicalAccess access){config.teleportAccessibilitySave="";String k=reportKey(d.type(),d.name());if(access==TeleportDestination.PhysicalAccess.UNKNOWN)config.teleportSpawnReports.remove(k);else config.teleportSpawnReports.put(k,access.name());dataRevision++;cachedPlan=null;config.save();}
    public synchronized void replaceSpawnReports(Map<String,String> reports){config.teleportSpawnReports=new HashMap<>(reports);dataRevision++;cachedPlan=null;}
