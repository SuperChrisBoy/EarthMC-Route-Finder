...
public final class TeleportViewerSettingsScreen extends Screen {
    ...
    @Override public void render(GuiGraphics g,int mx,int my,float d){int left=Math.max(12,(width-560)/2),right=Math.min(width-12,left+560);g.fill(left-12,16,right+12,height-8,0xF2080C10);g.fill(left-9,19,right+9,height-11,0xFA10171D);super.render(g,mx,my,d);g.drawCenteredString(font,title,width/2,30,0xFFFFFFFF);g.drawString(font,Component.translatable("earthmcroutefinder.teleport.primary_town"),left+18,64,0xFFE4E9ED,false);String[] keys={"earthmcroutefinder.teleport.settings.advanced_enabled","earthmcroutefinder.teleport.settings.command_action","earthmcroutefinder.teleport.settings.town_spawns","earthmcroutefinder.teleport.settings.nation_spawns","earthmcroutefinder.teleport.settings.route_line","earthmcroutefinder.teleport.settings.target_marker","earthmcroutefinder.teleport.settings.arrival_marker"};int py=102;for(String key:keys){g.drawString(font,Component.translatable(key),left+18,py,0xFFE0E6EA,false);if(key.endsWith("command_action")){if(cfg.teleportCommandAction==2)g.drawString(font,Component.translatable("earthmcroutefinder.settings.execute_command_warning"),left+288,py+20,0xFFFF5555,false);py+=40;}else py+=27;}g.drawString(font,Component.translatable("earthmcroutefinder.teleport.settings.hint"),left+18,py+12,0xFFAFBAC2,false);if(!RouteFinderMod.isOnEarthMcServer()){g.drawString(font,Component.translatable("earthmcroutefinder.safety.non_earthmc_warning"),left+18,332,0xFFFF5555,false);g.drawString(font,Component.translatable("earthmcroutefinder.safety.teleport_override"),left+18,356,0xFFFF7777,false);}if(!status.isBlank())g.drawString(font,status,left+18,height-48,0xFFFFCC66,false);}
    @Override public void onClose(){cfg.save();minecraft.setScreen(parent);}
}
