...
Every saved map edit automatically exports the full network with the active draft's changes to `EarthMC Ice Road Planner/json/full-map-*.json`. Unique filenames preserve previous exports and avoid overwriting exports from other sessions. Unchanged saves do not create duplicate files. Untouched roads are retained, imported roads are replaced by their edited versions, and station IDs are remapped to avoid collisions. Undo and redo also save the resulting map. Failed saves remain pending for retry.

New drafts retain their original upstream map snapshot across saves and undo/redo. Full-map exports use a three-way merge against the latest map loaded by the mod, including updates received while editing. Independent object fields merge automatically; overlapping changes (including delete-versus-edit) block export. Geometry and station-reference arrays are treated as whole units to avoid silently reconnecting roads. The draft is still saved, and a report containing the original snapshot, draft, and current upstream map is written under `EarthMC Ice Road Planner/conflicts/`. The editor displays **Export conflict**. Compare the report and reapply the intended edits in a new draft based on the updated map to resolve it. Legacy drafts without an original snapshot are preserved but must also be reapplied in a new draft before full-map export; the mod does not guess their baseline. Conflict checks use the most recently fetched network, not an additional fetch on every edit.

Use **Export > Website JSON** for a manually named full-map file accepted by the original ice-highways-map website. The planner autosave also includes website-compatible `lines` and `stations` for the active draft, alongside its lossless `drafts` library. Breaks and connector edges are converted to standard website branches in that view.
