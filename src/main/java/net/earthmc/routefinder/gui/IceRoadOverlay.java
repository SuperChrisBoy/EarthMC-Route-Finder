...
public final class IceRoadOverlay {
    ...
    private static void card(GuiGraphicsExtractor g,int sw,int sh,RouteFinderConfig cfg,IceRoadNetwork.Station s){int w=300,h=92,x=Math.max(8,sw-w-12),y=Math.max(8,sh-h-12);g.fill(x,y,x+w,y+h,0xF20A1419);g.text(Minecraft.getInstance().font,s.name(),x+8,y+7,0xFFFFFFFF,false);g.text(Minecraft.getInstance().font,Component.translatable("earthmcroutefinder.ice_station.coords",Math.round(s.x()),Math.round(s.z()),typeName(s.type())),x+8,y+21,0xFFB9C5CB,false);String detail=s.notes().isBlank()?(s.lines().isEmpty()?Component.translatable("earthmcroutefinder.ice_station.no_line_information").getString():String.join(" · ",s.lines().subList(0,Math.min(2,s.lines().size())))):s.notes();if(detail.length()>52)detail=detail.substring(0,49)+"...";g.text(Minecraft.getInstance().font,detail,x+8,y+35,0xFFDDE7ED,false);String[] keys={"unknown","accessible","blocked"};String[] values={"UNKNOWN","ACCESSIBLE","OBSTRUCTED"};for(int i=0;i<3;i++){int bx=x+8+i*95;boolean on=values[i].equals(access(cfg,s));g.fill(bx,y+57,bx+91,y+78,on?0xFF315E70:0xFF1B2931);g.centeredText(Minecraft.getInstance().font,Component.translatable("earthmcroutefinder.ice_station.status."+keys[i]),bx+45,y+63,on?0xFFFFFFFF:0xFFB9C5CB);}}
