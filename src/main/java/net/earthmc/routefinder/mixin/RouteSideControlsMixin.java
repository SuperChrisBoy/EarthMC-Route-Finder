...
public abstract class RouteSideControlsMixin {
    @Shadow private static void drawToggle(GuiGraphics g,Font font,int row,int top,String label,boolean active){throw new AssertionError();}
    @Shadow private static void drawTexturedButton(GuiGraphics g,int x,int y,int w,int h,String label,boolean active,int color){throw new AssertionError();}
    ...
    private static void routefinder$sideButtons(GuiGraphics g,Font font,int settingsY,CallbackInfo ci){
        int top=settingsY-8*23-7;
