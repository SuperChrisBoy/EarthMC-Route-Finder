...
public final class UiWorkflowSmoke {
    ...
    static void press(Screen screen,String label) throws ReflectiveOperationException{
        Button b=(Button)((Screen)screen).children().stream().filter(c->c instanceof Button v&&v.getMessage().getString().equals(label)).findFirst().orElseThrow();
        b.onPress(null);
    }
    ...
    static void verifyPlanner(Minecraft mc,Screen parent) throws ReflectiveOperationException {
        ...
        try{
            ...
            var boxes=mc.screen.children().stream().filter(c->c instanceof EditBox).map(c->(EditBox)c).toList();
            ...
            if(!(mc.screen instanceof CoordinatesScreen))throw new IllegalStateException("Map point skipped Y prompt");
            press(mc.screen,"Cancel");
            ...
            call(null,type,"place",new Class[]{double.class,double.class},100.0,100.0);
            ((EditBox)mc.screen.children().stream().filter(c->c instanceof EditBox).findFirst().orElseThrow()).setValue("-32");
            press(mc.screen,"Apply");
    ...
    public static void verify(Minecraft mc) throws ReflectiveOperationException{
        ...
        mc.setScreen(screen);
        var boxes=((Screen)screen).children().stream().filter(c->c instanceof EditBox).map(c->(EditBox)c).toList();
        ...
        if(mc.screen!=screen||applied[0]!=null)throw new IllegalStateException("Blank Y accepted");
        ...
        try{
            ...
            if(!(mc.screen instanceof CoordinatesScreen))throw new IllegalStateException("No height prompt");
            var y=(EditBox)mc.screen.children().stream().filter(c->c instanceof EditBox).findFirst().orElseThrow();
        ...
        }finally{set(planner,"placementY",oldY);mc.setScreen(parent);}
        verifyPlanner(mc,parent);
        var saves=new AccessibilitySavesScreen(parent);mc.setScreen(saves);
        ...
        try{
            ...
            var name=((Screen)saves).children().stream().filter(c->c instanceof EditBox).map(c->(EditBox)c).findFirst().orElseThrow();
            name.setValue("Overwrite smoke");press(saves,"Save current");
            if(!(mc.screen instanceof ConfirmScreen))throw new IllegalStateException("No overwrite warning");
            press(mc.screen,"No");
            ...
            press(saves,"Save current");press(mc.screen,"Yes");
