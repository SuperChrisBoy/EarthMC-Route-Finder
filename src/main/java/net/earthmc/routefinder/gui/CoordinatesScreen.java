...
public final class CoordinatesScreen extends Screen {
    ...
    @Override protected void init(){
        ...
        addRenderableWidget(Button.builder(Component.literal("Apply"),b->{
            try {XYZ v=XYZ.parse(values[0],values[1],values[2]);minecraft.gui.setScreen(parent);accept.accept(v);}
    ...
    @Override public void extractRenderState(GuiGraphicsExtractor g,int mx,int my,float delta){
        g.fill(0,0,width,height,0xF010171D);super.extractRenderState(g,mx,my,delta);
        int left=Math.max(8,(width-300)/2),top=Math.max(28,(height-164)/2);
        g.centeredText(font,title,width/2,top+4,0xFFFFFFFF);
        for(int i=0;i<3;i++)if(!heightOnly||i==1)g.text(font,new String[]{"X","Y","Z"}[i],left,top+30+(heightOnly?0:i*26),0xFFFFFFFF,false);
        g.text(font,font.plainSubstrByWidth(error,Math.min(300,width-16)),left,top+(heightOnly?56:108),0xFFFFAA66,false);
    }
