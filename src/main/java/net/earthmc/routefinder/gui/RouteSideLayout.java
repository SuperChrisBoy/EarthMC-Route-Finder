...
/** Two rows inserted after Counter and before Settings in the base column. */
public final class RouteSideLayout {
    public static final int EXTRA_HEIGHT=46;
    ...
    public static int hit(double x,double y,int top){
        ...
        for(int row=0;row<2;row++)if(y>=rowY(top,row)&&y<rowY(top,row)+20)return row;
        return -1;
    }
}
