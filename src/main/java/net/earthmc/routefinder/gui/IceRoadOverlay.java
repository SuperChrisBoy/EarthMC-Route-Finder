...
public final class IceRoadOverlay {
    ...
    public static void renderMinimap(GuiGraphics g,double playerX,double playerZ,double pixelsPerBlock,double angle,int mapX,int mapY,int size,boolean circular,RouteFinderConfig cfg){
    ...
    public static void renderSelectedRouteMinimap(GuiGraphics g,TeleportViewerOverlay.MinimapRoute route,double playerX,double playerZ,double pixelsPerBlock,double angle,int mapX,int mapY,int size,boolean circular,RouteFinderConfig cfg){
    ...
    private static void symbol(GuiGraphics g,int x,int y,String type,String access,boolean active,int size){var m=g.pose();m.pushMatrix();try{m.translate(x,y);float scale=size/14f;m.scale(scale,scale);symbolAtOrigin(g,type);}finally{m.popMatrix();}}
    /** Website-faithful SVG-derived glyph shared with the in-map highway planner. */
    static void drawStationSymbol(GuiGraphics g,int x,int y,String type,int size){symbol(g,x,y,type,"UNKNOWN",false,size);}
    private static void symbolAtOrigin(GuiGraphics g,String type){int c=0xFFFFFFFF;if(type.startsWith("jct"))hollowSquare(g,0,0,c);else if(type.startsWith("inter"))interchange(g,0,0,c,type.endsWith("1"));else if(type.startsWith("elev"))elevator(g,0,0,c,type.endsWith("ew"));else if(type.startsWith("semi"))halfRing(g,0,0,c,Character.getNumericValue(type.charAt(type.length()-1)));else{g.fill(-3,-3,4,4,0x333388FF);ring(g,0,0,c,0,12);}}
    /** Rasterized directly from the source map's 14x14 inter1/inter2 SVG paths. */
    private static void interchange(GuiGraphics g,int x,int y,int c,boolean inter1){
    ...
    private static void elevator(GuiGraphics g,int x,int y,int c,boolean ew){
    ...
    private static void rect(GuiGraphics g,int x,int y,int c,int x1,int y1,int x2,int y2){g.fill(x-7+x1,y-7+y1,x-7+x2,y-7+y2,c);}
